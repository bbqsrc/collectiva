...
const memberService = require("../../../../src/backend/services/memberService")
...
function getExpectedNewMember(residentialAddressId, postalAddressId, dateOfBirth) {
  const momentDate = moment(dateOfBirth, "DD/MM/YYYY").toDate()
  const member = fakeNewMember(residentialAddressId, postalAddressId, momentDate)
...
describe("memberService", () => {
  describe("createMember", () => {
    ...
    let createMemberStub, addressStub
  ...
  describe("verify", () => {
    ...
    it("should verify the member if email and hash match", (done) => {
      const hash = "1d225bd0-57b5-4b87-90fc-f76ddc997e57"

      updateMemberPromise.resolve({ dataValues: { email: "sherlock@holmes.co.uk", verified: moment().format(), id: "1" } })

      const memberToVerify = { dataValues: { email: "sherlock@holmes.co.uk", verified: null, id: "1" }, update: updateMemberStub }
  ...
  describe("findMembershipsExpiringOn", () => {
    ...
    it("should return an empty array if no memberships expiring on the date", (done) => {
      ...
      const expiredOn = moment().format("L")
      const query = { where: { lastRenewal: moment(expiredOn, "L").subtract(1, "year").toDate() }, attributes: ["id", "email"] }
  ...
  describe("renewMember", () => {
    ...
    it("should set the lastRenewal date if member was found", (done) => {
      const memberToRenew = { dataValues: { } }
